...
$secondChoice=$_POST['c'];
 if ($secondChoice== 'toyota' || 'honda' || 'suzuki' || ' mercedes' || 'nissan' || 'bmw' || 'audi' || 'rolls royce' || 'chevrolet' )
...
<img src=" <?php  echo $row['image'];  ?>" name="" style="padding: 5px; width: 270px; height: 186px; box-shadow:-1px 4px 26px 5px rgba(0,0,0,0.75); border-radius: 10px; ">
<div style="width: 360px; height: 180px; color:white; margin-left: 320px; padding: 10px; margin-top: -180px;  position: absolute;">
    <p style="margin-left: 15px; font-size: 18px; text-shadow: grey 0 0 10px;"><b>CAR NAME: <?php echo $row['cname'] ?> </b></p>
        <p style="margin-top: -10px; margin-left: 15px; font-size: 15px; color:white; text-decoration:underline;margin-top: 10px ">CAR MODEL: <?php echo $row['model']; ?><br> BRAND: <?php echo $row['brand']; ?> </p>
