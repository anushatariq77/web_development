   ...
   <table class="table" style="background: #560606; height:90%; width: 80%; margin-left: 8%; color:white; margin-top: 3%; opacity: 0.8; position: absolute;">
     ...
     for($i=1;$i<=$num;$i++)
     {
          ...
          <tr>
               ...
               <td><button onclick="myFunction(<?php echo $row['cid']; ?>)" type="submit" style="color: black;" name="<?php echo $row['cid']; ?>">Delete</button></td>
          </tr>
          
          <script>
function myFunction(delid){
   if ( confirm("Do you want to delete selected record?")){
        window.location.href='deletion.php?del_id='+delid+'';
        return true;
   }
}
     </script>
